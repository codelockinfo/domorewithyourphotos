...
var html_tag = document.documentElement,
	...
	lazyloadAgainEvt = new CustomEvent("lazyloadAgain"),
	...
	productVariantPickerEvt = new CustomEvent("productVariantPicker");
...
/*function loadModelViewer() {
	if (!html_tag.classList.contains('model-viewer-loaded')) {
		var model_tag = document.createElement('script');
		model_tag.src = 'https://cdn.shopify.com/shopifycloud/model-viewer/v1.10/model-viewer.js';
		model_tag.setAttribute('type', 'module');
		document.body.appendChild(model_tag);
		html_tag.classList.add('model-viewer-loaded');
	}
}*/
...
function toggle_dropdowns(el, selector) {
	...
	if (el.parentElement.classList.contains('toggle')) {
		...
		/*for (i = 0; i < j.length; i = i + 1) {
			Array.from(j[i].querySelectorAll('li.toggle')).forEach(function (em) {
...
function nav_burger() {
	...
	if (html_tag.classList.contains('m2a')) {
		...
		//console.log(nav_id.querySelectorAll('a:not(.toggle)')[1]);
		setTimeout(function () {
		...
		new_css('css-menu', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-menu.css?v=36590982183417486461691157453');
	}
}
...
function navSubHover(el) {
	...
	if (el.classList.contains('sub')) {
		Array.from(nav_id.querySelectorAll('.ul-inner')[0].children).forEach(function (el) {
			//el.parentElement.classList.remove('ul-hover');
...
window.addEventListener("nav", function(evt) {
	...
	/*if(nav_bar_id || nav_id){
		append_url(root_id, 'Close', 'overlay-content');
	}*/
	if (nav_id && !isMobile) {
	...
	if (nav_bar_id) {
		...
		if (nav_id) {
			...
			//if(nav_bar_id.hasAttribute('aria-label')){
			//nav_id_sub_cont.classList.add('nav-bar-element');
			//nav_id_sub_cont.innerHTML = '<a href="./" class="toggle wide">' + nav_bar_id.getAttribute('aria-label') + '</a> <ul></ul>';
			//nav_id_ul[0].prepend(nav_id_sub_cont);
			//}
		}
		// handle mouseovers
	...
	if (nav_id) {
		...
		if (nav_bar_id) {
			...
			/*Array.from(document.querySelectorAll('#nav > ul > li a.toggle-back.wide')).forEach(function (el) {
		...
		//nb.style.height = html_tag.offsetHeight - header_id.offsetHeight + 'px';
		nb.innerHTML += 'Close';
		...
		/*Array.from(nav_id.querySelectorAll('ul.inner li.sub')).forEach(function (el) {
			el.addEventListener('mouseover', function (e) {
				console.log('a');
			});
		});*/
...
function asyncCSS() {
	"use strict";
	new_css('async-css', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async.css?v=52509283729687325021691157451');
}
...
function asyncCSS() {
	"use strict";
	new_css('async-css', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async.css?v=52509283729687325021691157451');
	new_css('hovers-css', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-hovers.css?v=182661397761404903101691157452');
}
if (!isMobile) {
...
/*html_tag.addEventListener('mousemove', function () {
	loadHovers();
});*/
...
function validate_me(el) {
	"use strict";
	loadRes("//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/plugin-validator.js?v=71457463131436978931691157479", function () {
		if (typeof Validator === 'function') {
			...
			//console.log(el.handleFormSubmit)
			/*Array.from(el.querySelectorAll('input, textarea')).forEach(function (em) {
				if (em.value == "") {
		...
		new_css('form-validation-css', "//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-validation.css?v=135226482748542705341691157452");
	}, 'validator-loaded');
			...
			el.decr.addEventListener('click', function (e) {
				if (el.input.hasAttribute('min')) {
					if (el.value > parseFloat(el.input.getAttribute('min'))) {
						el.value -= step;
						if (el.value < parseFloat(el.input.getAttribute('min'))) {
							el.value = parseFloat(el.input.getAttribute('min'));
						}
					}
				} else {
					if (el.value > 1) {
			...
			el.incr.addEventListener('click', function (e) {
				if (el.input.hasAttribute('max')) {
					if (el.value < parseFloat(el.input.getAttribute('max'))) {
						el.value += step;
						if (el.value > parseFloat(el.input.getAttribute('max'))) {
							el.value = parseFloat(el.input.getAttribute('max'));
						}
					}
				} else {
					el.value += step;
...
// Countdown (product page) - legacy, don't use
/*var scheme_countdown = document.getElementsByClassName('s1cu');

function updateTimer(el, alias) {
	'use strict';
	var future = Date.parse(alias),
		diff = future - new Date(),
	...
	el.innerHTML = '<span class="count-item day" data-count="' + d + '">' + d + ' <span class="count-label">days</span></span> ' + '<span class="count-item hour" data-count="' + h + '">' + h + ' <span class="count-label">hours</span></span><span class="sep">:</span> ' + '<span class="count-item min" data-count="' + m + '">' + m + ' <span class="count-label">minutes</span></span><span class="sep">:</span> ' + '<span class="count-item sec" data-count="' + s + '">' + s + ' <span class="count-label">seconds</span></span>';
}
Array.from(scheme_countdown).forEach(function (el) {
	var inner = el.innerHTML;
	updateTimer(el, inner);
...
if (form_product_sticky.length) {
	Array.from(form_product_sticky).forEach(function (el) {
		...
		/*if(el.classList.contains('visible')){
			new_css('css-product-scrolled', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-product-scrolled.css?v=19869886983330127861691157453');
			html_tag.classList.add('f8ps-visible', 'f8ps-css');
		}*/
...
// 		"use strict";
/*el.addEventListener('submit', function (e) {
    new_css('form-validation-css', path_css + 'async-validation.css');
});*/
// 	});
// }
...
window.addEventListener("semanticSelect", function (evt) {
	...
	if (select_tag.length && !isMobile) {
		loadRes("//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/plugin-selects.js?v=55162161851225322351691157453", function () {
			"use strict";
			Array.from(select_tag).forEach(function (el) {
				...
				if (!isMobile) {
					...
					Array.from(el.parentNode.getElementsByClassName('bv_atual')).forEach(function (em) {
						em.addEventListener('click', function () {
							new_css('css-select', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-select.css?v=116768044393015328621691157452');
						});
				...
				// 		offset: false
				// 	});
				// 	var selectedValue = el.dataset.default;
				// 	if (selectedValue) {
				// 		for (var i = 0, count = el.options.length; i < count; i++) {
				// 			var option = el.options[i];
				// 			if (selectedValue == option.value || selectedValue == option.innerHTML) {
				// 				el.querySelector
				// 				bvsel.SetOption({
				// 				  type: "byValue",
				// 				  value: option.value
				...
				// 		}
				// 	}
				// 	Array.from(document.querySelectorAll('.bv_ul_inner a')).forEach(function (em) {
				// 		em.addEventListener('click', function () {
				// 			el.dispatchEvent(changeEvent);
				// 			if (em.closest('.form-group') !== null) {
				// 				em.closest('.form-group').classList.remove('is-invalid');
...
// new_css('css-panels', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-panels.css?v=48796026522504069801691157454');
if (module_panel.length) {
	function openPanel(id) {
		...
		if (document.getElementsByClassName('f8fl').length) {
			new_css('css-filters', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-filters.css?v=76219953153071283391691157454');
		}
		getStickyFooters();
		new_css('css-panels', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-panels.css?v=48796026522504069801691157454');
	}
...
window.addEventListener("imageCompare", function(evt) {
	...
	if (image_compare.length) {
		loadRes('//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/plugin-compare.js?v=108280108769912210341691157453', function () {
			"use strict";
			new_css('compare-css', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-compare.css?v=61498998280410013761691157451');
			Array.from(image_compare).forEach(function (el) {
				...
				if (el.querySelectorAll('img').length === 2) {
					...
					/*if(el.getAttribute('data-label-before') === '' || el.getAttribute('data-label-after') === ''){
						opt_labels = false;
					} else*/
...
window.addEventListener("inputDate", function(evt) {
	...
	if (input_date.length && !isMobile) {
		...
		new_css('datepicker-css', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-datepicker.css?v=165252557381951514801691157454');
		if (datepicker_langs.includes(global_lang)) {
			...
			loadRes('//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/plugin-datepicker.js?v=65836746582345829981691157454', function () {
...
function overlayClose(n) {
	...
	/*Array.from(search_input).forEach(function (en) {
		en.value = '';
	});*/
}
	...
	if (search_id) {
		...
		if (search_id.classList.contains('compact')) {
			var search_compact_cont = document.createElement('span'),
				v_cm; //,
			//link = createElementWithClass('label', 'search-compact-toggle');
			if (nav_user_id) {
			...
			//search_compact_cont.appendChild(link);
			...
			if (search_id.classList.contains('compact-mobile')) {
		...
		Array.from(search_input).forEach(function (el) {
			...
			el.addEventListener('focus', function () {
				...
				new_css('css-search', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-search.css?v=73015925010885649431691157453');
			});
		});
		...
		/*Array.from(search_id.querySelectorAll('.search-back')).forEach(function (el) {
			"use strict";
			el.addEventListener('click', function (e) {
				compactSearch();
				e.preventDefault();
		...
		Array.from(document.querySelectorAll('label[for="search_main"]')).forEach(function (el) {
			"use strict";
			el.addEventListener('click', function (e) {
				compactSearch();
				e.preventDefault();
		...
		/*Array.from(document.querySelectorAll('a[href="#search"]')).forEach(function (el) {
			"use strict";
			el.addEventListener('click', function (e) {
				compactSearch();
				e.preventDefault();
...
window.addEventListener("schemeTooltip", function (evt) {
	...
	if (scheme_tooltip.length) {
		// new_css('css-tooltips', "//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-tooltips.css?20688");
		Array.from(scheme_tooltip).forEach(function (el, in1) {
			...
			el.innerHTML = '<span class="s1tt-text">' + el.innerHTML + '</span>';
			var icon = createElementWithClass('i', 'icon-info');
			icon.setAttribute('aria-hidden', 'true');
			append_url(el, 'Tooltip', 's1tt-tip');
			append_url(el, 'Popup', 's1tt-popup');
			el.appendChild(icon);
			el.classList.add('ready');
			el.getElementsByClassName('s1tt-popup')[0].setAttribute('data-popup', 'tip-' + nr);
			el.getElementsByClassName('s1tt-tip')[0].addEventListener('click', function (e) {
				if (el.classList.contains('toggle')) {
					el.classList.remove('toggle');
					if (el.closest('p') !== null) {
						el.closest('p').classList.remove('s1tt-active');
					}
					if (el.closest('li') !== null) {
						el.closest('li').classList.remove('s1tt-active');
					}
				} else {
					Array.from(scheme_tooltip).forEach(function (el) {
						el.classList.remove('toggle');
					});
					el.classList.add('toggle');
					if (el.closest('p') !== null) {
						el.closest('p').classList.add('s1tt-active');
					}
					if (el.closest('li') !== null) {
						el.closest('li').classList.add('s1tt-active');
					}
				}
				e.preventDefault();
			});*/
		...
		/*enquire.register('screen and (min-width: 761px)', function () {
			document.addEventListener('click', function (event) {
				if (!(event.target.getAttribute('class') === 's1tt-tip')) {
					Array.from(scheme_tooltip).forEach(function (el) {
						el.classList.remove('toggle');
					});
				}
...
var loadPopup = function (id) {
	"use strict";
	loadRes('//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/plugin-popups.js?v=183952906476440921941691157480', function () {
		var allPopups = document.querySelectorAll('[class^="popup-"]:not(html):not(.ready, .initialized-popup)');
		if (allPopups.length > 0 /*&& !document.documentElement.classList.contains('spi')*/) {
			...
			Array.from(document.querySelectorAll('.popup-blocker a.close')).forEach(function (el) {
				...
				el.addEventListener('click', function (e) {
					...
					if (popup_delay_not_blocker.length) {
						...
						// setTimeout(function () {
						// 	loadPopup(popup_delay_not_blocker[0].getAttribute('data-title'));
						// 	popupFocus(popup_delay_not_blocker[0].getAttribute('data-title'));
						// }, popup_delay_not_blocker[0].getAttribute('data-popup-delay'));
					}
				});
			});
		}
		/*var popup_blocker = document.getElementsByClassName('popup-blocker');
		if (popup_blocker.length) {
			Array.from(popup_blocker.getElementsByClassName('close')).forEach(function (el) {
				el.addEventListener('click', function (e) {
					console.log('aa');
				});
			});
...
window.addEventListener("listScrollable", function(evt) {
	...
	if (list_collection.length) {
		Array.from(list_collection).forEach(function (el) {
			...
			// Show more links (cart)
			/*Array.from(el.querySelectorAll('li.link')).forEach(function (el) {
				el.addEventListener('click', function (e) {
					...
					if (data_img.length && first_img !== undefined) {
						...
						Array.from(data_img).forEach(function (en) {
							en.addEventListener('mouseover', function () {
								...
								/*second_img.setAttribute('src', en.getAttribute('data-img'));
                                if (second_img.hasAttribute('srcset')) {
                                    second_img.removeAttribute('srcset');
                                }*/
							});
						});
					}
...
// .n6as (collection)
var nav_aside = document.querySelectorAll('.n6as a.toggle');
if (nav_aside.length) {
...
window.addEventListener("rangeSlider", function(evt) {
	...
	if (input_range.length) {
		...
		loadRes("//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/plugin-sliders.js?v=141160130285589861841691157451", function () {
			"use strict";
			new_css('form-sliders-css', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-ui-sliders.css?v=72896717569740218131691157451');
			Array.from(input_range).forEach(function (el) {
				...
				/*if (el.querySelectorAll('span.label').length) {
                    el.classList.add('has-label');
                }*/
				if (!el.classList.contains('slider-is-here')) {
					el.querySelectorAll('.range-inner').forEach(function (el) {
						...
						el.noUiSlider.on('change', function (handle) {
							if (handle) {
						...
						Array.from(el.querySelectorAll('[role="slider"]')).forEach(function (em) {
...
// Masonry, .l4ft (index)
/*window.addEventListener("masonry", function(evt) {
	var list_featured = document.getElementsByClassName('l4ft');
	if (list_featured.length) {
		loadRes("//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/plugin-masonry.js?v=19905838331864423561691157452", function () {
			"use strict";
			Array.from(list_featured).forEach(function (el) {
				if (el.classList.contains('cols')) {
					...
					//var aa = event.target.closest('.l4ft');
					//if (aa !== null) {
						//.destroy();
					//}
				}
...
window.addEventListener("stickyNav", function(evt) {
	...
	if (header_id) {
		...
		// if (nav_top_id !== null || html_tag.classList.contains('t1nt')) {
		...
		if (html_tag.classList.contains('t1sn')) {
			...
			//console.log(le.getBoundingClientRect().top);
			//console.log(window.scrollY);
			//console.log(sa);
			distance_counter.style.top = window.scrollY + sa + 'px';
			//distance_counter.style.top = le.getBoundingClientRect().top + sa + 'px';
		...
		/*const observer2 = new IntersectionObserver(ip, {
			rootMargin: (le.getBoundingClientRect().top + window.scrollY)+'px',
			threshold: 1
		});
		observer2.observe(root_id);*/
	}
	...
	if (top_bar) {
		...
		top_bar.querySelector('a.close').addEventListener('click', function (e) {
			...
			/*if (document.querySelectorAll('#distance-counter') != 'undefined') {
		...
		if (Cookies.get('notice', { sameSite: 'none', secure: true }) === 'closed') {
			...
			new_css('announcement-css', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-announcement.css?v=179453402485244085771691157452');
...
window.addEventListener("mobileFilters", function(evt) {
	...
	if (form_sort.length) {
		Array.from(form_sort).forEach(function (el) {
			...
			if (el.classList.contains('mobile-sticky')) {
				...
				if (trg !== undefined) {
					if (!html_tag.classList.contains('t1sn') && header_main && !header_main.classList.contains('no-sticky')) {
						if (header_id) {
							ofs = header_id.clientHeight;
						}
						if (header_id && nav_top_id) {
							ofs = header_id.clientHeight - nav_top_id.clientHeight;
						}
					}
					dst = '-' + parseFloat(ofs + 16) + 'px 0px ' + parseFloat(ofs + 16) + 'px 0px';

					function io2(entries) {
						entries.map((entry) => {
							if (el.getBoundingClientRect().top < header_id.clientHeight)
								if (entry.isIntersecting) {
									...
									//if (header_id) {
										//trg.style.top = ofs + 'px';
									//}
								}
						});
					}

					new IntersectionObserver(io2, {
						rootMargin: dst,
						threshold: 0.7
					}).observe(el);
				}*/
	...
	if (form_filter.length) {
		Array.from(form_filter).forEach(function (el) {
			...
			//append_url(root_id, 'Close', 'f8fl-toggle', './', true);
...
window.addEventListener("countdown", function(evt) {
	...
	if (countdown_tag.length) {
		loadRes('//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/plugin-countdown.js?v=30852516867416231701691157453', function () {
			"use strict";
...
if (list_count.length) {
	loadRes('//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/plugin-countup.js?v=23403240054145588091691157452', function () {
		"use strict";
		Array.from(list_count).forEach(function (em) {
			Array.from(em.querySelectorAll('span:first-child')).forEach(function (el) {
				...
				/*if (el.innerHTML.indexOf('+') > -1) {
					clone_me.classList.add('has-plus');
					str = str.replace('+', '');
				}*/
			...
			//console.log(em.getBoundingClientRect().y < window.innerHeight);
...
function updateSliders(el) {
	"use strict";
	if (el.classList.contains('s4wi') /*&& !html_tag.classList.contains('theme-editor')*/) {
...
window.addEventListener("alerts", function(evt) {
	var list_alerts = document.querySelectorAll('.l4al:not(.inline)');
...
if (link_print.length) {
	Array.from(link_print).forEach(function (el) {
		"use strict";
		el.addEventListener('click', function (e) {
			new_css('css-print', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-print.css?v=98634256293469202081691157451');
			setTimeout(function () {
				window.print();
			}, 400);
			e.preventDefault();
	...
	function createFancyboxAndShowItem(itemIndex, fancyName) {
		...
		fbox = new Fancybox(showItems, {
			...
			/*caption: function (fancybox, carousel, slide) {
				return (
					all_items[itemIndex].getAttribute('data-caption')
				);
			},*/
			on: {
				/*done: (fancybox, slide) => {
				...
				ready: function (_fancybox) {
					...
					//console.log(carousel.$content.childNodes[0])
					//console.log(_fancybox.$carousel.children[0].querySelectorAll('.fancybox__slide:first-child .fancybox__content > *'))
				...
				"Carousel.change": function (_fancybox, carousel, slide) {
					...
					//console.log(_fancybox.options.Html.video.autoplay)
	...
	if (data_fancybox.length) {
		Array.from(data_fancybox).forEach(function (el, index) {
			...
			el.addEventListener('click', function (e) {
				...
				loadRes("//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/plugin-fancybox.js?v=53225757861153421831691157452", function () {
					new_css('css-fancybox', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-fancybox.css?v=123126178332476150511691157452');
...
/*var form_search = document.getElementsByClassName('f8se');
if (form_search.length) {
	Array.from(form_search).forEach(function (el) {
		"use strict";
...
window.addEventListener("hotspots", function (evt) {
	...
	if (list_hotspots.length) {
		new_css('css-hotspots', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-hotspots.css?v=149621784538869558091691157451');
...
window.addEventListener("maqruee", function (evt) {
	...
	if (module_maqruee.length) {
		loadRes("//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/plugin-typewritter.js?v=141057276794469966851691157454", function () {
			Array.from(module_maqruee).forEach(function (el) {
				...
				/*if (el.classList.contains('size-s')) {
					el.parentElement.classList.add('has-size-s');
				}
				if (el.classList.contains('size-m')) {
					el.parentElement.classList.add('has-size-m');
				}
				if (el.classList.contains('size-l')) {
					el.parentElement.classList.add('has-size-l');
				}*/
				if (!el.classList.contains('type')) {
					el.style.setProperty('--items', li.length);

					//wrap(el, document.createElement('div'), 'm6kn-wrapper');
					//el.innerHTML = '<div class="inner">' + el.innerHTML + '</div>';
					/*for (var i = 0; i < 2; i++) {
                        clone_li = el.children[0].cloneNode(true);
                        el.appendChild(clone_me);
                    }*/
		...
		new_css('css-marquee', '//cdn.shopify.com/s/files/1/0099/8693/2814/t/37/assets/async-marquee.css?v=119070173001214993091691157453');
	}
});
...
/*var list_product_model = document.querySelectorAll('.l4pr model-viewer');
if (list_product_model.length) {
	loadModelViewer();
}*/
...
/*var autoplay_videos = document.querySelectorAll('video[playsinline]');
if (autoplay_videos.length) {
	Array.from(autoplay_videos).forEach(function (el) {
		"use strict";
		el.play()
	});
}*/
...
window.addEventListener("lazyloadAgain", function(event) {
	var element = event.details.section;
	var img_lazy = element.querySelectorAll('img[data-src]:not(.dont), picture[data-srcset]:not(.dont)');
	for (var i = 0; i < img_lazy.length; i = i + 1) {
...
var ajaxCart = (function(module) {
	...
	handleCartPanel = function(resultsMarkup, openCartPanel, undoRemove) {
		...
		window.dispatchEvent(formValidateEvt);
		updateCartCount();
		lazyloadAgainEvt.details = { section: sideCartContainer };
			...
			button.addEventListener('click', function(e) {
				...
				fetch(button.getAttribute('href'))
					...
					.then((text) => {
						...
						lazyloadAgainEvt.details = { section: collectionSection };
						saveLoadMoreAnchor();
						window.dispatchEvent(lazyloadAgainEvt);
						window.dispatchEvent(ratingsEvt);
						window.dispatchEvent(semanticInputEvt);
						...
						window.dispatchEvent(semanticSelectEvt);
						window.dispatchEvent(productVariantPickerEvt);
						ajaxCart.init();
						quickShop.init();
					})
					.catch((error) => {
		...
		var processFilters = function() {
			...
			fetch(filterUrl)
					...
					.then((text) => {
						...
						window.dispatchEvent(modulePanelEvt);
						lazyloadAgainEvt.details = { section: collectionSection };
						if (layout == 'drawer') { lazyloadAgainEvt.details = {section: sidebarGlobal}; }
						window.dispatchEvent(lazyloadAgainEvt);
						window.dispatchEvent(ratingsEvt);
						window.dispatchEvent(semanticInputEvt);
						window.dispatchEvent(schemeTooltipEvt);
				...
				setTimeout(function (token) {
					...
					fetch(new_url + "&sections=" + renderSections)
						...
						.then((text) => {
							...
							if (resultsMarkupForm.getElementsByClassName('no-pl-px') != null) {
								Array.from(resultsMarkupForm.getElementsByClassName('no-pl-px')).forEach(function (el) {
							...
							window.dispatchEvent(formZindexEvt);
							window.dispatchEvent(dataChangeEvt);
							lazyloadAgainEvt.details = { section: productFormSection };
							window.dispatchEvent(lazyloadAgainEvt);
							ajaxCart.init();
							if (hasSticky && (productFormTemplate.endsWith('main-product')) && (!isQuickshop) ) {
...
// Productpage sticky buy bar
// window.addEventListener("productBuybarBtn", function (evt) {
// 	var form_product_sticky_btn = document.querySelector('.f8ps button');
// 	if (form_product_sticky_btn != null) {
// 		form_product_sticky_btn.addEventListener('click', function() {
// 			document.querySelector('[id^="main-product-form"] button[type="submit"]').click();
// 		});
// 	}
// });
// window.dispatchEvent(productBuybarBtnEvt);
				...
				fetch(url)
					...
					.then((text) => {
						...
						if (count == calls) {
							...
							el.classList.remove('hidden');
							window.dispatchEvent(listScrollableEvt);
							lazyloadAgainEvt.details = { section: el };
							window.dispatchEvent(lazyloadAgainEvt);
...
var quickShop = (function(module) {
	...
	openQuickshop = function(el) {
		...
		fetch(quickshopUrl)
			...
			.then((text) => {
				...
				resultsMarkup.querySelector('header > h1, header > h2, header > h3').innerHTML = '<a href="'+ quickshopUrl +'">' + resultsMarkup.querySelector('header > h1, header > h2, header > h3').innerHTML + '</a>';
	...
	handleQuickshopPanel = function(resultsMarkup, el) {
		...
		window.dispatchEvent(schemeTooltipEvt);
		window.dispatchEvent(popupsEvt);
		lazyloadAgainEvt.details = { section: quickshopContainer };
		window.dispatchEvent(lazyloadAgainEvt);
...
// Productcards variant picker add to cart
window.addEventListener("productVariantPicker", function (evt) {
	var variantPickers = document.querySelectorAll('.l4cl figure .check.plain')

	if (variantPickers.length) {
		Array.from(variantPickers).forEach(function (el) {
			let radio = el.querySelectorAll('input[type="radio"]');
			if(radio) {
				radio.forEach(function (input) {
					input.addEventListener('click', function (e) {
						let addToCartBtn = el.parentElement.querySelector('button[type="submit"]');
						if (addToCartBtn) {
							addToCartBtn.click();
						}
					})
