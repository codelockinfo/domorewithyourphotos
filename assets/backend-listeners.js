document.addEventListener("" + "shopify:section:load", function (event) {
  ...
  if (
    event.srcElement.getElementsByClassName("product-recommendations").length
  ) {
    setTimeout(function () {
  ...
  if (
    event.detail.sectionId.endsWith("main-collection") ||
    event.detail.sectionId.endsWith("main-search")
  ) {
  ...
  if (event.srcElement.getElementsByClassName("m6kn").length) {
    window.dispatchEvent(maqrueeEvt);
  }
  if (
    event.srcElement.querySelectorAll("[data-shopthelook]").length ||
    event.srcElement.getElementsByClassName("l4ft").length
  ) {
    hidePanels();
...
document.addEventListener("shopify:block:select", function (event) {
  ...
  if (
    section.classList.contains("shopify-section-header") &&
    event.srcElement.classList.contains("sub")
  ) {
  ...
  if (
    ...
    tabs
      ...
      .classList.add(active_class, custom_active_class);
    tabs
      .querySelector("div[data-index='" + tabIndex + "']")
      .classList.remove("hidden", custom_active_class);
  }
});
